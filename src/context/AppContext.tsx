...
export function AppProvider({ children }: AppProviderProps) {
  ...
  const [savedComparisonMetas, setSavedComparisonMetas] = useState<{
    [id: number]: ComparisonMetaData;
  }>({});
  ...
  function updateDataInStorage(id: number, data: IDBStorageItem) {
    ...
    updateRequest.addEventListener('success', () => {
      ...
      setSavedComparisonMetas((old_data_list) => {
        old_data_list[updateRequest.result as number] = {
          id: updateRequest.result as number,
          title: data.title,
        };
  ...
  function onIDBConnectionSuccess() {
    ...
    getSavedComparisonMetas(IDB_CONNECTION_REQ.result)
      .then(console.log)
      .catch(console.error);
  }
  ...
  useEffect(() => {
    ...
    IDB_CONNECTION_REQ.addEventListener(
      'upgradeneeded',
      onIDBConnectionUpgradeNeeded
    );
    return () => {
      IDB_CONNECTION_REQ?.removeEventListener('error', onIDBConnectionError);
      IDB_CONNECTION_REQ?.removeEventListener(
        'success',
        onIDBConnectionSuccess
      );
    };
  }, []);
  ...
  useEffect(() => {
    localStorage.setItem(
      LOCAL_STORAGE_KEY,
      JSON.stringify(savedComparisonMetas)
    );
