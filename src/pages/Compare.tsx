...
let uniqueDifferenceInterval: ReturnType<typeof setTimeout> | undefined =
  undefined;
...
function uniqueDifferenceFunctionV2(differenceObject: Difference) {
  ...
  right.extra
    .filter(
      (x, index) =>
        !right.extra.find((y, _index) => y.startsWith(x) && index != _index)
    )
    .forEach((path) => {
...
export default function Compare(props: CompareProps) {
  ...
  const [uniqueDiff, setUniqueDiff] = useState<
    { pathLeft?: string; pathRight?: string }[]
  >([]);
  ...
  const [currentDifferenceIndex, setCurrentDifferenceIndex] =
    useState<number>(0);
  const [loadingState, setLoadingState] = useState<boolean>(false);
  const [
    uniqueDifferenceWorker,
    { status: differenceWorkerStatus, kill: killDifferenceWorker },
  ] = useWorker(uniqueDifferenceFunctionV2);
  const [
    differenceFinderWorker,
    { status: differenceFinderWorkerStatus, kill: killDifferenceFinderWorker },
  ] = useWorker(differenceV2, {
    localDependencies() {
      return { ...utils, cache: {} };
    },
  });
  const [
    leftSortWorker,
    { status: leftSortWorkerStatus, kill: killLeftSortWorker },
  ] = useWorker(customSort, {
    localDependencies() {
      return { ...jsonabc };
    },
  });
  const [
    rightSortWorker,
    { status: rightSortWorkerStatus, kill: killRightSortWorker },
  ] = useWorker(customSort, {
    localDependencies() {
      return { ...jsonabc };
    },
  ...
  const [currentlyViewing, setCurrentlyViewing] = useState<
    { json: any; title: string } | undefined
  >(undefined);
  ...
  useEffect(() => {
    ...
    if (props?.initLeftContent || props?.initRightContent) {
      return;
    }
    if (id === undefined) {
  ...
  function differenceFinderRunner(leftjson: any, rightjson: any) {
    console.log(
      'Running difference finder function runner',
      differenceFinderWorkerStatus
    );
    ...
    if (leftjson && rightjson) {
      ...
      differenceFinderWorker(leftjson, rightjson)
        .then((_diff) => {
          setDifference(_diff);
  ...
  function uniqueDifferenceFunctionRunner() {
    console.log(
      'Running unique difference function runner',
      differenceWorkerStatus
    );
    ...
    uniqueDifferenceWorker(differenceObject)
      .then((result) => {
  ...
  useEffect(() => {
    ...
    functionsToRunAfterRender.forEach((fn) =>
      fn(leftRefEditor.current!, rightRefEditor.current!)
    );
    return () => {
      setFunctionsToRunAfterRender([]);
    };
  }, [
    leftRefEditor.current,
    rightRefEditor.current,
    functionsToRunAfterRender,
  ]);
  ...
  function onSave() {
    ...
    if (assignedId) {
      database.comparisons
        .update(assignedId, { data: data_to_save })
        .then((v) => {
          // console.log('v1', v);
          // navigate(`/compare/${v}`);
        })
  ...
  return (
    <Container css={{ padding: 0, position: 'relative' }} xl>
      <>
        ...
        <div className={styles.grid}>
            ...
            <JSONPane
              ...
              childProps={{
                ...
                onClassName: (path, value) => {
                  ...
                  if (
                    differenceObject?.left.different.find((x) =>
                      x.startsWith(pathString)
                    )
                  ) {
                    // console.log(pathString, differenceObject.left.different);
                    return styles.different;
                  } else if (
                    differenceObject?.left.missing.find((x) =>
                      x.startsWith(pathString)
                    )
                  ) {
                    ...
                    differenceObject?.left.extra.find((x) =>
                      x.startsWith(pathString)
                    )
                  ) {
          ...
          <div className={styles.mid}>
            <div>
              ...
              <Button.Group
                disabled={uniqueDiff.length === 0}
                alt="Difference Navigation"
                label="Difference Navigation"
                size="xs"
              >
                <Button
                  onClick={focusOnNextDifference}
                  disabled={uniqueDiff.length === 0}
                  alt="Next Difference"
                ...
                <Button
                  onClick={focusOnPreviousDifference}
                  disabled={uniqueDiff.length === 0}
            ...
            <div style={{ marginTop: 50 }}>
              <Button
                disabled={
                  false ||
                  !(
                    ((leftRefEditor.current?.get() as any)?.json ||
                      (leftRefEditor.current?.get() as any)?.text) &&
                    ((rightRefEditor.current?.get() as any)?.json ||
                      (rightRefEditor.current?.get() as any)?.text)
                  )
                }
                size="xs"
                onClick={onSave}
          ...
          <div id={rightId} className={styles.col}>
            <JSONPane
              ...
              childProps={{
                content: props?.initRightContent,
                mode: rightMode,
                ...
                onClassName: (path, value) => {
                  ...
                  if (pathString === '$.') return '';
                  if (
                    differenceObject?.right.different.find((x) =>
                      x.startsWith(pathString)
                    )
                  ) {
                    // console.log(pathString, differenceObject.left.different);
                    ...
                    differenceObject?.right.missing.find((x) =>
                      x.startsWith(pathString)
                    )
                  ) {
                    // console.log(pathString, differenceObject.left.missing);
                    ...
                    differenceObject?.right.extra.find((x) =>
                      x.startsWith(pathString)
                    )
                  ) {
                    // console.log(pathString, differenceObject.left.extra);
        ...
        {loadingState && (
          <Loading
            css={{ position: 'absolute', width: '100vw', height: '100vh' }}
          />
        )}
      </>
    </Container>
